...
void cOneRow::redrawState() {
	...
	for (int i = 0; i < (int)enemy.size(); ++i) {
		animal_vehical* curEnemy = enemy[i];
		printNewEnemy(curEnemy->getPos(), curEnemy->array2(), curEnemy->getWidth(), curEnemy->getHeight());
	}
}
...
bool cOneRow::pushEnemy(animal_vehical* newEnemy) {
	if (!direction) {
	...
	printNewEnemy(newEnemy->getPos(), newEnemy->array2(), newEnemy->getWidth(), newEnemy->getHeight());
	return true;
}

vector<animal_vehical*> cOneRow::getEnemy() {
	return enemy;
}
...
int cOneRow::moveToNextState(int t) {
	...
	vector <animal_vehical*> newEnemy;
	...
	for (int i = 0; i < (int)enemy.size(); ++i) {
		...
		// Print Enemy
		bool canPrint = printNewEnemy(curEnemy->getPos(), curEnemy->array2(), curEnemy->getWidth(), curEnemy->getHeight()); 
		if (!canPrint) {
			curEnemy->goOutMap();
			++nDelete;
		}
